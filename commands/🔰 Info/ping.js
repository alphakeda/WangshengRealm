    ...
    try {
      ...
      message.reply({embeds: [new MessageEmbed()
        ...
        msg.edit({embeds: [new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          .setFooter(client.getFooter("It Takes longer, because i am getting my host ping!", client.user.displayAvatarURL()))
          .setTitle(handlemsg(client.la[ls].cmds.info.ping.m2, { botping: Math.floor(client.ws.ping + newtime), ping: Math.floor(newtime) + "ms", wsping: Math.floor(client.ws.ping)})+`\n\n<:online:983992323820118076> Database Ping: \`${dbping}ms\``)
        ]}).catch(console.error)
      })
    } catch (e) {
