...
module.exports = (client, preindex) => {
  ...
  client.on("interactionCreate", async (interaction) => {
      ...
      try {
        ...
        if (data) {
          if (data.state != "closed") {
            if(!interaction.replied) await interaction.reply({ content: `❌ **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true }).catch(console.warn);
            else await interaction.editReply({ content: `❌ **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true }).catch(console.warn);
            return
          }
        }
      } catch {
     ...
     * CREATE THE CHANNEL
     */
    if(!interaction.replied) await interaction.reply({ content: `<:idle:983992319520931861> **Creating your Ticket...** (Usually takes 0-2 Seconds)`, ephemeral: true }).catch(console.warn);
    else await interaction.editReply({ content: `<:idle:983992319520931861> **Creating your Ticket...** (Usually takes 0-2 Seconds)`, ephemeral: true }).catch(console.warn);
