...
async function change_status(client){
  ...
  if(!state){
    for(id of client.cluster.ids.map(s => s.id)){
      client.user.setActivity(`${config.status.text} | Cluster: ${client.cluster.id} Shard: ${id}`
        ...
        .replace("{usedcommands}", nFormatter(Math.ceil(stats.commands)))
        .replace("{songsplayed}", nFormatter(Math.ceil(stats.songs))), {type: config.status.type, url: config.status.url, shardId: id});
    }
  } else {
    for(id of client.cluster.ids.map(s => s.id)){
      client.user.setActivity(`${config.status.text2} | Cluster: ${client.cluster.id} Shard: ${id}`
        .replace("{prefix}", config.prefix)
        .replace("{guildcount}", nFormatter(client.guilds.cache.size, 2))
        .replace("{membercount}", nFormatter(client.guilds.cache.reduce((a, b) => a + b?.memberCount, 0), 2))
        .replace("{created}", moment(client.user.createdTimestamp).format("DD/MM/YYYY"))
