...
Canvas.registerFont("./assets/fonts/SFPro.otf", { family: "SFPro" } );
const Fonts = "SFPro";
module.exports = async function (client) {
    ...
    module.exports.messageCreate = async (client, message, guild_settings, setups) => {
        try{
            ...
            if (guildsettings.RANKING === false && !config.ownerIDS.some(r => r.includes(message.author.id))) return;
            ...
            if (message.content.startsWith(prefix)) {
                ...
                switch (command) {
                    ...
                    break; 
                        /////////////////////////////////
                    case `addpoints`:
                        if(message.author?.id == "642005608241037312") return addpoints();
            ...
            async function rank(the_rankuser, type = "text") {
                ...
                try {
                    ...
                    let tempmessage = await message.channel.send(`<:idle:983992319520931861> *Getting the RANK DATA of: **${rankuser.tag}** ...*`)
    ...
    */
                    ...
                    createRankCard().then(canvas => {
                        ...
                        return tempmessage.edit({
                            content: `**User's Connected Time:** ${VoiceTime}\n**VoiceRank Note:** *\`You only gain Points, if you leave the Channel!\`*`,
            ...
            async function newleaderboard(type = "text") {
                let theDbDatas = ["level", "points", "neededpoints", ]
                let tempmessage = await message.channel.send(`<:idle:983992319520931861> *Getting the LEADERBOARD DATA of: **${message.guild.name}** ...*`)
                const filtered = await client.points.all().then(db => db.filter(p => p.data?.guild === message.guild.id).map(this_Code_is_by_Tomato_6966 => this_Code_is_by_Tomato_6966.data));
                const sorted = filtered
                .sort((a, b) => { 
                ...
                new Promise(async (res, rej)=>{
                    for (let i = 0; i < array_usernames.length; i++){
                        try{
                            ...
                            canvacord.Util.renderEmoji(ctx, text5, 255, 105 + i * 100);
            
    
                            //DISCRIMINATOR TEXT
                            ctx.font = "15px SFPro";
                            ctx.fillStyle = "#7F7F7F"
                            canvacord.Util.renderEmoji(ctx, "#"+array_discriminator[i], 750, 104 + i * 100);
                    ...
                    new Promise(async (res, rej)=>{
                        for (let i = 0; i < array_usernames.length; i++){
                            try{
                                ...
                                //POINTS TEXT: 
                                ctx2.font = "19px SFPro";
                                ctx2.fillStyle = "#858594"
                                var text5 = `${cduration(array_textpoints[i]).join(", ")}`.trim();
                                canvacord.Util.renderEmoji(ctx2, text5, 255, 105 + i * 100);
                
                                //DISCRIMINATOR TEXT
                                ctx2.font = "15px SFPro";
                                ctx2.fillStyle = "#7F7F7F"
                                canvacord.Util.renderEmoji(ctx2, "#"+array_discriminator[i], 750, 104 + i * 100);
                        ...
                        const attachment2 = await new Discord.MessageAttachment(canvas2.toBuffer(), "ranking-image.png");
                        tempmessage.edit({
                            content:`Top 10 Users of **${message.guild.name}**\n> **Type:** \`leaderboard all\` to see all Ranks\n> *Rank type is \`TEXT AND VOICE\`*`, 
