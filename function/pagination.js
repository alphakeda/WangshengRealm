    ...
    pagination: async (options) => {
      ...
      const defaultStyles = {
        ...
        if (embeds.length > 1) {
          ...
          sentMsg = await message.reply({embeds: [embeds[0]], components: [firstPageRow] }) 
        } else {

          disabledPageRowButtons = [
            first.setDisabled(true), 
            previous.setDisabled(true), 
          ...
          disabledPageRow = new MessageActionRow()
            .addComponents(disabledPageRowButtons)

          sentMsg = await message.reply({embeds: [embeds[0]], components: [disabledPageRow] }) 
        }
        ...
        collector.on('end', i => {
          ...
          sentMsg.edit({components: [disabledPageRow]
        }).catch(() => {})
      })
    }
};