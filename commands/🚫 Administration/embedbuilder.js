...
let embedEditing = async(button) => {
    ...
    if(id.startsWith(`builder`)) {
        ...
        button?.message.edit({content : `<:idle:983992319520931861> **Please send me Your Input now!**`,
        ...
        if(noInputFinal) {
            ...
            input = await button?.channel.awaitMessages({filter,  max: 1, time: 120000, errors: ['time'] }).catch(e => {
