...
module.exports = {
      ...
      run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
        ...
        if(link == "https://cdn.discordapp.com/attachments/826089183050465280/977431649748938792/unknown-2.png?size=4096") { message.reply({embeds : [
        new MessageEmbed()
        .setColor(es.color)
        .setImage(await link)
        .setDescription('You found an Easter Egg!')
        .setFooter(`Requested by: ${message.author.username}`, message.author.displayAvatarURL({ dynamic: true }))
        .setTimestamp()
    ]}).catch(() => {}) }else { message.reply({embeds : [
        new MessageEmbed()
        .setColor(es.color)
        .setImage(await link)
