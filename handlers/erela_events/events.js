...
module.exports = (client) => {
  client.manager
    ...
    .on("trackStart", async (player, track) => {
      try {
        try {
          client.stats.add(`global.songs`, 1)
        } catch (e) {}
        let edited = false;
...
function generateQueueEmbed(client, player, track){
  var embed = new MessageEmbed().setColor(ee.color)
    ...
    embed.setFooter({ text: `Requested by: ${player.queue.current.requester.tag}`, iconURL: player.queue.current.requester.displayAvatarURL({dynamic: true})});
