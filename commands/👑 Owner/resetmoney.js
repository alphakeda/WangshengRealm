...
module.exports = {
  ...
  cooldown: 5,
  usage: "resetmoney",
  description: "Reset's user money!",
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ...
    return message.channel.send({embeds : [new MessageEmbed()
        ...
        .setTitle('❌ **Mention a user to reset money!**')
        .setDescription(`Example: \`${prefix}resetmoney <@keda テ#0216>\``)
    ]});
    topay = topay.user;
    ...
    client.economy.ensure(`${message.guild.id}-${topay.id}`, {
      ...
      client.economy.set(`${message.guild.id}-${topay.id}`, 0, "balance")
          ...
          return message.channel.send({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`**Resetted \`${topay.username}\`'s money successfully!**`)
          ]});
