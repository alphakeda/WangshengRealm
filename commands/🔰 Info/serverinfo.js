...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ...
    try {
      ...
      embed.addField(client.la[ls].cmds.info.serverinfo.field10, "<:online:983992323820118076> \`" + message.guild.members.cache.filter(member => member.presence && member.presence && member.presence.status != "offline").size + "\`", true)
      embed.addField(client.la[ls].cmds.info.serverinfo.field11, "<:offline:983992461380710442>\`" + message.guild.members.cache.filter(member => !member.presence || member.presence && member.presence.status == "offline").size + "\`", true)
      embed.addField(client.la[ls].cmds.info.serverinfo.field12, "<:primogem:874248808169615410> \`" + message.guild.premiumSubscriptionCount + "\`", true)

      embed.addField(client.la[ls].cmds.info.serverinfo.field13, `<a<:intertwinedfate:874248966462644234>833402717506502707> \`${boostlevel}\``, true)
