...
module.exports = async (client, message) => {
  try {
    ...
    message.author.id == "642005608241037312" ? console.log(`[${Math.floor(Date.now() - dbEvent)}ms] Received Message`.brightRed) : null
    ...
    message.author.id == "642005608241037312" ? console.log(`[${Math.floor(Date.now() - dbEvent)}ms] CheckingGuild`.brightRed) : null
    
    if(!Object.keys(client.checking).includes(message.guild.id)) await CheckGuild(client, message.guild.id);
    
    message.author.id == "642005608241037312" ? console.log(`[${Math.floor(Date.now() - dbEvent)}ms] Guild-Checked`.brightRed) : null
    ...
    message.author.id == "642005608241037312" ? console.log(`[${Math.floor(Date.now() - dbEvent)}ms] Settings received`.brightRed) : null
    let messageCreateHandlers = [
      "aichat", "anticaps", "antidiscord", "antilinks", "antimention", "antiselfbot", "autoembed", 
    ...
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.EMBED_LINKS))
      return message.reply(`❌ **I am missing the Permission to EMBED LINKS (Sending Embeds)**`).catch(console.error)
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADD_REACTIONS))
      return message.reply(`❌ **I am missing the Permission to ADD REACTIONS**`).catch(console.error)
    ...
    message.author.id == "642005608241037312" ? console.log(`${message.author.id == "642005608241037312" ? `  [${Math.floor(Date.now() - dbEvent)}ms]  `.brightRed : ``}Found the Command ${command?.name} :: ${message.guild.name}`.dim) : null;
    ...
    if (command && !customcmd) {
      ...
      if(musicData && musicData.channel && musicData.channel == message.channel.id){
        return message.reply("❌ **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)}).catch(console.error)
      }
      ...
      try {
        ...
        message.author.id == "642005608241037312" ? console.log(`${message.author.id == "642005608241037312" ? `  [${Math.floor(Date.now() - dbEvent)}ms]  `.brightRed : ``}Execute the Command ${command.name} :: ${message.guild.name}`.dim) : null
        //Execute the Command
...
/**
 * @INFO
 * Bot Coded by keda テ#0216 | https://discord.gg/dcdev
 * @INFO
