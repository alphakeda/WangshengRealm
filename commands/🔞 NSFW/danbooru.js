...
module.exports = {
  ...
  category: "🔞 NSFW",
  usage: "danbooru",
  description: "Searches a query from danbooru image board",
  type: "anime",
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ...
    return message.reply({embeds: [new MessageEmbed()
      ...
      .setDescription(`Usage: \`${prefix}danbooru <query>\``)
    ]});
    
    var query = message.content.split(/\s+/g).slice(1).join(" ");
    await booru.search('db', [query], {
        random: true
      })
      .then(booru.commonfy)
      .then(images => {
        for (let image of images) {
         if(!image.common.file_url) { message.reply(eval(client.la[ls]["cmds"]["nsfw"]["danbooru"]["variable6"])) } else { 
