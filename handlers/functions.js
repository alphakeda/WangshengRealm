...
async function autoplay(client, player, type) {
  ...
  try {
    ...
    console.log("QUEUE IS AUTOPLAYING on GUILD: ".yellow + player.guild)
    const previoustrack = player.get("previoustrack") || player.queue.current;
    if (!previoustrack)  return console.log("ERROR AUTOPLAYING on GUILD: ".red + player.guild);
    ...
    if (!response || response.loadType === 'LOAD_FAILED' || response.loadType !== 'PLAYLIST_LOADED') {
      console.log("ERROR AUTOPLAYING on GUILD: ".red + player.guild);
      return await autoplay(client, player, type);
      /*
      ...
      client.channels.cache.get(player.textChannel).send({embeds: [embed]}).catch(e => console.log("THIS IS TO PREVENT A CRASH".yellow));
          ...
          try {
            ...
            if (player.queue.size === 0) {
              ...
              client.channels.cache
                ...
                .send({embeds: [embed]}).catch(e => console.log("ERROR CODE 1 on GUILD: ".red + player.guild))
              try {
                client.channels.cache
                  .get(player.textChannel)
                  .messages.fetch(player.get("playermessage")).then(async msg => {
                    try {
                      await delay(7500)
                      msg.delete().catch(() => console.log("ERROR CODE 2 on GUILD: ".red + player.guild));
                    } catch {
                      console.log("ERROR CODE 3 on GUILD: ".red + player.guild) 
                    }
                  }).catch(() => null);
              } catch (e) {
                console.error(e)
    ...
    console.log("AUTOPLAYING on GUILD: ".yellow + player.guild)
    addautoplaytracks(client, player, type, previoustrack, response)
    //player.queue.add(response.tracks.filter(r => r.identifier != previoustrack.identifier)[0]);
  } catch (e) {
    console.error(e)
  }
}

async function addautoplaytracks(client, player, type, previoustrack, response, tracksNumber = 0) {
  const MaxAutoplayTracks = settings.MaxAutoplayTracks;
  try {
    if(tracksNumber < MaxAutoplayTracks) {
    player.queue.add(response.tracks.filter(r => r.identifier != previoustrack.identifier)[tracksNumber]);
    console.log(`Adding track: `.yellow + `${tracksNumber + 1} track`)
    return await addautoplaytracks(client, player, type, previoustrack, response, ++tracksNumber)
    } else 
    console.log("ADDED TRACKS on GUILD: ".brightGreen + player.guild)
    return player.play();
  } catch(e) {
    console.error(e)
  }
...
async function swap_pages_data(client, message, description, TITLE, T_cmd = "Unkown") {
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author?.id)
        return b?.reply({content: `❌ **Only the one who typed the cmd is allowed to react!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
          collector.resetTimer();
          //b?.reply("***Swapping a PAGE FORWARD***, *please wait 2 Seconds for the next Input*", true)
      ...
      if (guildid) {
        ...
        await dbEnsure(client.settings, guildid, {
          ...
          antidiscord: {
            ...
            whitelistedchannels: [],
            whitelistedlinks: [
              "https://dsc.gg/wangshengrealm",
              "discord.gg/243uR5gVvN"
            ],
            mute_amount: 2,
          },
          embed: {
