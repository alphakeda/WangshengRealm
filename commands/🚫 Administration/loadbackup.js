...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
        ...
        if(message.guild.me && !message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR)){
            return message.reply(`❌ **I am missing the ADMINISTRATOR Permission in ${message.guild.name}!**`)
        }
        ...
        message.channel.send({
            ...
            collector.on('collect', button => {
                if (button?.user.id === cmduser.id) {
                    collector.stop();
                    button?.reply({content: `<:idle:983992319520931861> Now loading the Backup from \`${server.name}\` to \`${message.guild.name}\`!`}).catch(() => null)
                    // Create the backup
